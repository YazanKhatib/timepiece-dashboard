        ...
        if (cookies.token)
          ...
          await axios({
            ...
            setCookie(
              'token',
              {
                accessToken: response.data.data.loginAdmin.accessToken,
                refreshToken: response.data.data.loginAdmin.refreshToken,
              },
              { expires: addToDate(new Date(), 'minutes', 29) },
            );
            config.headers['Authorization'] =
              'Bearer ' + response.data.data.loginAdmin.accessToken;
          });
        }
      }
      return config;
