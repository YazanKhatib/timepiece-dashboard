  ...
  return (
    <ul className="statistics-filter">
      {props.supportedTypes
        ? props.supportedTypes.map((item, index) => {
            const period = t(SupportedPeriods[item].toLowerCase());
            return (
              <li
                onClick={() => {
                  setActive(index);
                  if (props.trigger)
                    props.trigger(SupportedPeriods[item].toLowerCase());
                }}
                className={active === index ? 'active' : ''}>
                <i>{period[0]}</i>
                <i>{t(period.slice(1))}</i>
              </li>
            );
          })
        : ''}
